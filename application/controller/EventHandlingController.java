...
import application.view.EditRoomDialog;
...
//			     executor.submit(r);
        ...
        btn_filmsave.setOnAction((event) -> {
            ...
            if (!tf_filmduration.getText().isEmpty() && !tf_filmtitle.getText().isEmpty() && !coverpath.isEmpty()
                ...
                if (lbl_film.getText().equals("Create new film")) {
                    ...
                    Movie editedFilm = new Movie();
                    //editedFilm.setDescription(ta_filmdesc.getText());
                    // Überprüft ob Filmlänge eine Zahl ist
                    try {
                        editedFilm.setId(movie.getId());
                        editedFilm.setMinutes(Integer.parseInt(tf_filmduration.getText()));
                        editedFilm.setName(tf_filmtitle.getText());
        ...
        btn_editfilm.setOnAction((event) -> {
            if (loadFilmList() != null) {
                movie = choosePopupFilm(loadFilmList(), "Edit");
                if (movie != null) {
                    backToMenu(false);
                    pane_film.setVisible(true);
                    pane_film.setDisable(false);
                    tf_filmduration.setText(Integer.toString(movie.getDurationInMinutes()));
                    tf_filmtitle.setText(movie.getTitle());
                    ta_filmdesc.setText(movie.getDescription());
                    Image pic = new Image("File:" + movie.getImagePath());
                    iv_filmcover.setImage(pic);
                    coverpath = movie.getImagePath();
        ...
        btn_deletefilm.setOnAction((event) -> {
            if (loadFilmList() != null) {
                movie = choosePopupFilm(loadFilmList(), "Delete");
                if (movie != null) {
                    returncode = controller.deleteFilm(movie);
        ...
        btn_createshow.setOnAction((event) -> {
            ...
            lbl_show.setText("Create new presentation");
        ...
        btn_showsave.setOnAction((event) -> {
            ...
            if (filmname != null && roomname != null && startDate != null && !startTime.isEmpty() &&  checkStartTime() && checkStartDate()) {
                if (lbl_show.getText().equals("Create new presentation")) {
                ...
                } else if (lbl_show.getText().equals("Edit presentation")) {
                    movie = controller.getFilmByName(lv_film.getSelectionModel().getSelectedItem());
                    Presentation editedShow = new Presentation();
                    Date date = null;
                    ...
                    try{
                        editedShow.setId(presentation.getId());
                        editedShow.setDuration(movie.getMinutes());
                        editedShow.setMovie(movie);
                        editedShow.setRoom(controller.getRoomByName(lv_room.getSelectionModel().getSelectedItem()));
                        editedShow.setStartDateTime(date);
                        editedShow.setEndDateTime(controller.showList.getEndTime(date, movie));
        ...
        btn_editshow.setOnAction((event) -> {
            if (loadShowList() != null) {
                presentation = choosePopupShow(loadShowList(), "Edit");
                if (presentation != null) {
                    loadEditShow(presentation);
                }
            } else {
                message.showMsg("i27");
                backToMenu(true);
    ...
    private void saveReservation() {
        ...
        for (int i = 0; i < seatList.size(); i++) {
            ...
            if (seat.isReserved()) {
                reservationList.add(seat.getRow() + "-" + seat.getSeatNumber());
            }
        }
